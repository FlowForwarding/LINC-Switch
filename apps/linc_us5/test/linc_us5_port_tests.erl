...
send_table() ->
    ...
    meck:expect(linc_us5_routing, spawn_route, fun(_) -> ok end),
...
send_flood() ->
    ?assertEqual(bad_port, linc_us5_port:send(pkt(), flood)).

send_all() ->
    ?assertEqual(ok, linc_us5_port:send(pkt(), all)),
    %% wait because send to port is a gen_server:cast
    timer:sleep(500),
    ?assertEqual(2, meck:num_calls(linc_us5_port_native, send, '_')).

...
bad_table_id() ->
    ...
    ?assertEqual({error,{flow_mod_failed,bad_table_id}},
                 linc_us3_flow:modify(?SWITCH_ID, FlowModAdd)).
                        ...
                        [#ofp_instruction_write_actions{
                            ...
                            actions =
                                [#ofp_action_output{port = 15,
                                                    max_len = 1400}]}]},
    %% Add flow
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowModAdd)).
...
add_flow(TableId, Flags) ->
    ...
    [#flow_entry{id=Id,
        ...
        linc_us3_flow:get_flow_table(?SWITCH_ID, TableId),
...
add_non_overlapping_flows(Flags) ->
    ...
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowModAdd2)),
...
add_overlapping_flow_check_overlap() ->
    ...
    ?assertEqual({error,{flow_mod_failed,overlap}},
                 linc_us3_flow:modify(?SWITCH_ID, FlowModAdd2)),
...
add_exact_flow(Flags) ->
    ...
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowModAdd1)),
    %% Get FlowId
    [#flow_entry{id=FlowId1}] = linc_us3_flow:get_flow_table(?SWITCH_ID,
                                                             TableId),

    %% Increments flow counters
    PacketSize = 1024,
    linc_us3_flow:update_match_counters(?SWITCH_ID, TableId,
                                        FlowId1, PacketSize),
    ...
    [#flow_entry{id = FlowId2,
                 match = M2,
                 instructions = I2}] = linc_us3_flow:get_flow_table(?SWITCH_ID,
                                                                    TableId),
...
modify_strict(Flags) ->
    ...
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowModAdd1)),
    %% Get FlowId
    [#flow_entry{id=FlowId}] = linc_us3_flow:get_flow_table(?SWITCH_ID,
                                                            TableId),
    ...
    %% Modify flow
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowMod)),
...
modify_cookie_no_match() ->
    ...
    %% Add first flow
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowModAdd1)),
    ?assertMatch([#flow_entry{}], linc_us3_flow:get_flow_table(?SWITCH_ID,
                                                               TableId)),
...
delete_strict() ->
    ...
    [#flow_entry{}] = linc_us3_flow:get_flow_table(?SWITCH_ID, TableId),
    %% Delete flow
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowDel)),
...
delete_all_tables() ->
    ...
    ?assertMatch([#flow_entry{}], linc_us3_flow:get_flow_table(?SWITCH_ID, 2)),
    ...
    ?assertEqual([],linc_us3_flow:get_flow_table(?SWITCH_ID, 2)).
...
delete_where_group() ->
    ...
    ?assertEqual(ok, linc_us3_flow:delete_where_group(?SWITCH_ID, 3)),

    ?assertMatch([#flow_entry{match=Match2,
                              instructions=Instructions2}],
                 linc_us3_flow:get_flow_table(?SWITCH_ID, 1)).
...
update_lookup_counter() ->
    ...
    ?assertEqual(ok, linc_us3_flow:update_lookup_counter(?SWITCH_ID, TableId)),
...
update_match_counter() ->
    ...
    %% Increments flow counters
    PacketSize = 1024,
    ?assertEqual(ok, linc_us3_flow:update_match_counters(?SWITCH_ID,
                                                         TableId,
                                                         FlowId,
                                                         PacketSize)),
...
update_bad_match_counter() ->
    ...
    FlowId = undefined,
    PacketSize = 1024,
    ?assertEqual(ok, linc_us3_flow:update_match_counters(?SWITCH_ID, TableId,
                                                         FlowId,PacketSize)),
    FlowEntryCounters = linc:lookup(?SWITCH_ID, flow_entry_counters),
...
empty_flow_stats() ->
    StatsReq = ofp_v3_utils:flow_stats(
                 [{table_id, 1}],
                 [{in_port,6}, {eth_dst,<<0,0,0,0,0,8>>}]),
    ?assertEqual(#ofp_flow_stats_reply{stats=[]},
                 linc_us3_flow:get_stats(?SWITCH_ID, StatsReq)).
...
empty_aggr_stats() ->
    ...
    ?assertEqual(#ofp_aggregate_stats_reply{packet_count=0,
                                            byte_count=0,
                                            flow_count=0},
                 linc_us3_flow:get_aggregate_stats(?SWITCH_ID, StatsReq)).
...
aggr_stats_1_table() ->
    ...
    [#flow_entry{id=FlowId1},
     #flow_entry{id=FlowId2}] = linc_us3_flow:get_flow_table(?SWITCH_ID,
                                                             TableId),
...
aggr_stats_all_tables() ->
    ...
    [#flow_entry{id=FlowId1}] = linc_us3_flow:get_flow_table(?SWITCH_ID, 1),
    [#flow_entry{id=FlowId2}] = linc_us3_flow:get_flow_table(?SWITCH_ID, 2),
...
empty_table_stats() ->
    ?assertMatch(#ofp_table_stats_reply{stats=[#ofp_table_stats{}|_]},
                 linc_us3_flow:get_table_stats(?SWITCH_ID,
...
get_default_table_config() ->
    ?assertEqual(controller, linc_us3_flow:get_table_config(?SWITCH_ID, 2)).

set_table_config() ->
    ?assertEqual(controller, linc_us3_flow:get_table_config(?SWITCH_ID, 2)),
    ?assertEqual(ok, linc_us3_flow:table_mod(?SWITCH_ID,
                                             #ofp_table_mod{table_id=2,config=drop})),
    ?assertEqual(drop, linc_us3_flow:get_table_config(?SWITCH_ID, 2)).

set_all_table_config() ->
    ?assertEqual(ok, linc_us3_flow:table_mod(?SWITCH_ID, #ofp_table_mod{table_id=2,config=drop})),
    ?assertEqual(ok, linc_us3_flow:table_mod(?SWITCH_ID, #ofp_table_mod{table_id=2,config=drop})),
    ?assertEqual(ok, linc_us3_flow:table_mod(?SWITCH_ID, #ofp_table_mod{table_id=all,
                                                            config=continue})),
    ?assertEqual(continue, linc_us3_flow:get_table_config(?SWITCH_ID, 2)),
    ?assertEqual(continue, linc_us3_flow:get_table_config(?SWITCH_ID, 3)),
    ?assertEqual(continue, linc_us3_flow:get_table_config(?SWITCH_ID, 4)).
...
idle_timeout() ->
    FlowModAdd1 = ofp_v3_utils:flow_add(
                    ...
                    [{write_actions,[{group,3}]}]),
    ?assertEqual(ok, linc_us3_flow:modify(?SWITCH_ID, FlowModAdd1)),
    [#flow_entry{id=FlowId}] = linc_us3_flow:get_flow_table(?SWITCH_ID, 1),
    timer:sleep(100),
    [#flow_entry{id=FlowId}] = linc_us3_flow:get_flow_table(?SWITCH_ID, 1),

...
mock([instructions | Rest]) ->
    ...
    ok = meck:expect(linc_us4_instructions, apply,
                     fun(Pkt, _) ->
                             {stop, Pkt}
                     end),
    mock(Rest);
...
check_output_on_ports() ->
    [{Pkt, PortNo}
     || {_, {_, send, [Pkt, PortNo]}, ok} <- meck:history(linc_us4_port)].

check_output_to_groups() ->
    [{Pkt, GroupId}
     || {_, {_, apply, [Pkt, GroupId]}, ok} <- meck:history(linc_us4_group)].

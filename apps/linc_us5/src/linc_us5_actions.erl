...
apply_list(Pkt, [#ofp_action_group{group_id = GroupId} | Rest], SideEffects) ->
    linc_us5_groups:apply(GroupId, Pkt),
...
apply_list(#linc_pkt{packet = P} = Pkt,
    ...
    P2 = linc_us5_packet:find_and_edit(
           P, mpls_tag,
           fun(T) ->
...
apply_list(#linc_pkt{packet = P} = Pkt, [#ofp_action_dec_mpls_ttl{} | Rest],
    ...
    try 
        ...
        throw:{error, invalid_ttl} = Error ->
            linc_us5_port:send(Pkt#linc_pkt{packet_in_reason=invalid_ttl},
                               controller),
            Error
    end;
...
apply_list(#linc_pkt{packet = P} = Pkt, [#ofp_action_dec_nw_ttl{} | Rest],
    ...
    try
        ...
        throw:{error,invalid_ttl}=Error ->
            linc_us5_port:send(Pkt#linc_pkt{packet_in_reason=invalid_ttl}, controller),
            Error
    end;
...
apply_list(#linc_pkt{packet = P} = Pkt, [#ofp_action_copy_ttl_in{} | Rest],
    ...
    P2 = case Tags of
             ...
             [#ipv4{ttl = OutermostTTL}, #ipv4{}] ->
                 linc_us5_packet:find_and_edit_skip(
                   P, ipv4,
                   fun(T) ->
...
apply_list(#linc_pkt{packet = P} = Pkt,
    ...
    {ISID, IsPreviousPBB} = case linc_us5_packet:find(P, pbb) of
                                not_found ->
                                    {<<1:24>>, false};
                                {_, PreviousPBB} ->
    ...
    IPCP = case linc_us5_packet:find(P, ieee802_1q_tag) of
               not_found ->
                   0;
               {_, PreviousVLAN} ->
...
apply_list(#linc_pkt{packet = P} = Pkt,
  ...
  when EtherType =:= 16#8847;
    ...
    FindOldMPLS = linc_us5_packet:find(P, mpls_tag),
    SetTTL = case linc_us5_packet:find(P, ipv4) of
                 not_found ->
                     case FindOldMPLS of
                         not_found ->
                             0;
    ...
    case FindOldMPLS of
        not_found ->
            ...
            InsertAfter = case linc_us5_packet:find(P, ieee802_1q_tag) of
                              not_found ->
                                  ether;
                              _ ->
                                  ieee802_1q_tag
...
apply_list(#linc_pkt{packet = P} = Pkt,
    ...
    P2 = case linc_us5_packet:find_and_edit(P, mpls_tag, PopMPLSHeader) of
             Unmodified when Unmodified =:= P ->
                 Unmodified;
             Modified ->
                 BeforeMPLSTag = case linc_us5_packet:find(P, ieee802_1q_tag) of
                                     not_found ->
                                         ether;
                                     _ ->
                                         ieee802_1q_tag
                                 end,
                 linc_us5_packet:find_and_edit(Modified, BeforeMPLSTag,
                                               ModifyEtherType)
         end,
    apply_list(Pkt#linc_pkt{packet = P2}, Rest, SideEffects);
...
apply_list(#linc_pkt{packet = Packet} = Pkt,
    ...
    Packet2 = linc_us5_packet:set_field(F, Packet),

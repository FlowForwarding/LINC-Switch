...
queue_stats_request() ->
    ...
    StatsRequest4 = #ofp_queue_stats_request{port_no = ValidPort,
                                             queue_id = all},
    StatsReply4 = linc_us5_queue:get_stats(?SWITCH_ID, StatsRequest4),
    ...
    StatsRequest5 = #ofp_queue_stats_request{port_no = any,
                                             queue_id = 1},
    StatsReply5 = linc_us5_queue:get_stats(?SWITCH_ID, StatsRequest5),
    ...
    StatsRequest6 = #ofp_queue_stats_request{port_no = any,
                                             queue_id = all},
    StatsReply6 = linc_us5_queue:get_stats(?SWITCH_ID, StatsRequest6),
...
sending() ->
    PortId = 1,
    QueueId = 1,
    ?assertEqual(ok, linc_us5_queue:send(?SWITCH_ID, PortId, QueueId, <<>>)).
...
min_rate() ->
    ...
    ?assertEqual(ok,
                 linc_us5_queue:set_min_rate(?SWITCH_ID, PortId, QueueId, 500)).
...
max_rate() ->
    ...
    ?assertEqual(ok,
                 linc_us5_queue:set_max_rate(?SWITCH_ID, PortId, QueueId, 500)).
...
setup() ->
    ...
    linc_us5_port:initialize(?SWITCH_ID, Config),
    Pid.

teardown(Pid) ->
    linc_us5_port:terminate(?SWITCH_ID),

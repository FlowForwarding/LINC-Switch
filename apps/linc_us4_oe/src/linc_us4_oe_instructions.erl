...
-module(linc_us4_oe_instructions).
...
apply2(#linc_pkt{switch_id = SwitchId} = Pkt,
    ...
    {NewPkt, TerminationType} = case linc_us4_oe_meter:apply(SwitchId,
                                                          MeterId, Pkt) of
                                    drop ->
                                        {Pkt, drop};
                                    {continue, P} ->
...
apply2(Pkt,
    ...
    {NewPkt, _SideEffects} = linc_us4_oe_actions:apply_list(Pkt, Actions),
...
apply2(#linc_pkt{actions = OldActions} = Pkt,
       ...
       stop) ->
    %% From Open Flow spec 1.3.1 page 15:
    %% Merges the specified action(s) into the current action set (see 5.10).

-module(linc_us4_oe_port_load_regulator_tests).
...
load_regulation_should_not_be_activated() ->
    ...
    %% and we assume that linc_us4_oe_port_load_regulator:limit_load_if_necesseary/2
    %% won't execute longer than acceptable delay (as defined in linc_us4_oe_port
    %% by ACCEPTABLE_DELAY_IN_MILSECS)

    %% WHEN
    UpdatedPortState =
        linc_us4_oe_port_load_regulator:limit_load_if_necessary(ControlMsg,
                                                             PortState),
...
load_regulation_message_should_be_received_periodically()->
    ...
    %% WHEN
    Tref = linc_us4_oe_port_load_regulator:schedule_periodic_check(?SWITCH_ID,
                                                                ?PORT_NO),
    ...
    receive
        success ->
            ?assertEqual(
               ok, linc_us4_oe_port_load_regulator:cancel_periodic_check(Tref));
        fail ->

     ...
     {"Applying Meters",
      {foreach,
       ...
       [{"Apply meter, no match", fun apply_none/0},
        ...
        {setup,
         fun() -> meck:new(linc_us5_packet),
                  meck:expect(linc_us5_packet, decrement_dscp,
                              fun(Pkt, _) -> Pkt end) end,
         fun(_) -> meck:unload(linc_us5_packet) end,
...
setup() ->
    linc:create(?ID),
    {ok, Pid} = linc_us5_meter_sup:start_link(?ID),
    unlink(Pid),
    Pid.

teardown(Pid) ->
    linc_us5_meter_sup:stop(?ID),

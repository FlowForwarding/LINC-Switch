...
%% @copyright 2012 FlowForwarding.org
-module(linc_us4_oe_instructions_tests).
...
meter() ->
    ...
    {_, NewPacket1} = linc_us4_oe_instructions:apply(Packet, Instructions1),
    ...
    {_, NewPacket2} = linc_us4_oe_instructions:apply(Packet, Instructions2),
...
apply_actions() ->
    ...
    ?assertMatch({stop, Pkt},
                 linc_us4_oe_instructions:apply(Pkt, [ApplyActions])).
...
clear_actions() ->
    ...
    ClearActions = #ofp_instruction_clear_actions{},
    {_, NewPacket} = linc_us4_oe_instructions:apply(SomePacket, [ClearActions]),
...
write_actions() ->
    ...
    {_, NewPacket} = linc_us4_oe_instructions:apply(SomePacket, [WriteActions]),
...
write_metadata() ->
    ...
    {_, NewPacket1} = linc_us4_oe_instructions:apply(Packet1, [WriteMetadata]),
    ...
    Packet2 = #linc_pkt{fields = #ofp_match{
                                    fields = [#ofp_field{
                                                 name = metadata,
                                                 value = OldMetadata}]}},
    {_, NewPacket2} = linc_us4_oe_instructions:apply(Packet2, [WriteMetadata]),
...
goto_table() ->
    ...
    ?assertEqual({{goto, 5}, #linc_pkt{}},
                 linc_us4_oe_instructions:apply(#linc_pkt{}, [GotoTable])).

empty() ->
    ?assertEqual({stop, #linc_pkt{}},
                 linc_us4_oe_instructions:apply(#linc_pkt{}, [])).

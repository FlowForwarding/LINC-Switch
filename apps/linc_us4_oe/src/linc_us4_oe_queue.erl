...
attach_all(SwitchId, PortNo, SendFun, QueuesConfig) ->
    ...
    Sup = linc:lookup(SwitchId, linc_us4_oe_queue_sup),
...
initialize(SwitchId) ->
    ...
    QueueSup = {linc_us4_oe_queue_sup,
                {linc_us4_oe_queue_sup, start_link, [SwitchId]},
                permanent, 5000, supervisor, [linc_us4_oe_queue_sup]},
    supervisor:start_child(linc:lookup(SwitchId, linc_us4_oe_sup), QueueSup).
...
init([SwitchId, [{PortNo, QueueNo} = Key, PortRateDesc, ThrottlingETS,
    ...
    History = linc_us4_oe_sliding_window:new(?HIST_BUCKET_COUNT, ?HIST_BUCKET_SIZE),
...
sleep_and_send(MyKey, MinRate, MaxRate, PortRate, ThrottlingETS,
    ...
    History1 = linc_us4_oe_sliding_window:refresh(History),
    TotalTransfer = linc_us4_oe_sliding_window:total_transfer(History1),
    HistoryLenMs = linc_us4_oe_sliding_window:length_ms(History1),
    ...
    try
        ...
        timer:sleep(PauseMs),
        SendFun(Frame),
        History2 = linc_us4_oe_sliding_window:bump_transfer(History1, FrameSize),
        History2
    catch
        E1:E2 ->
...
match_queue(SwitchId, PortNo, PortMatch, QueueMatch) ->
    case linc_us4_oe_port:is_valid(SwitchId, PortNo) of
        false ->

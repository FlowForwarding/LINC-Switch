...
switch_config_request_reply_test_() ->
    {setup,
     ...
     [
      {foreach,
       fun mocked_us5_backend_setup/0,
       fun(_) -> ok end,
       [
...
default_switch_config(State) ->
    {"Test if the switch initial config is set correctly",
     fun() ->
             [begin
                  {_, ConfigReply, _} = linc_us5:ofp_get_config_request(
    ...
    {"Test if the switch config is set correctly",
     fun() ->
             [begin
                  ...
                  {_, NewState} = linc_us5:ofp_set_config(
                                    State,
...
config_request_reply_setup() ->
    mocked = linc_us5_test_utils:mock(?MOCKED),
...
config_request_reply_teardown(_) ->
    unmocked = linc_us5_test_utils:unmock(?MOCKED),

...
with_ofconfig() ->
    ...
    application:set_env(linc, backend, linc_us4_oe),
...
default_switch_config(State) ->
    {"Test if the switch initial config is set correctly",
     fun() ->
             [begin
                  {_, ConfigReply, _} = linc_us4_oe:ofp_get_config_request(
    ...
    {"Test if the switch config is set correctly",
     fun() ->
             [begin
                  ...
                  {_, NewState} = linc_us4_oe:ofp_set_config(
                                    State,
...
config_request_reply_teardown(_) ->
    unmocked = linc_us4_oe_test_utils:unmock(?MOCKED),

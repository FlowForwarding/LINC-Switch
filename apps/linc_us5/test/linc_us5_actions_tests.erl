...
action_set_precedence() ->
    ...
    Output = linc_us5_actions:apply_set(Pkt),
...
action_set_drop() ->
    ...
    Output1 = linc_us5_actions:apply_set(Pkt1),
    ...
    Output2 = linc_us5_actions:apply_set(Pkt2),
...
action_group() ->
    ...
    ?assertEqual({Pkt, [{group, GroupId, Pkt}]},
                 linc_us5_actions:apply_list(Pkt, [ActionGroup])),
    ?assert(check_if_called({linc_us5_groups, apply, 2})).
...
action_complex_1() ->
    ...
    ?assertEqual({PktExpected, [{output, Port, PktExpected}]},
                 linc_us5_actions:apply_list(Pkt, [Action1, Action2])),
    ?assert(check_if_called({linc_us5_port, send, 2})),
...
check_action(Action, Packet, NewPacket) ->
    ...
    {Pkt2, ?NO_SIDE_EFFECTS} = linc_us5_actions:apply_list(Pkt, [Action]),
    ?assertEqual(NewPkt, Pkt2).

check_action_error(Action, Packet, Error) ->
    Pkt = #linc_pkt{packet = Packet},
    ?assertEqual(Error, linc_us5_actions:apply_list(Pkt, [Action])).

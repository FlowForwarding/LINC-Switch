    ...
    case group_get(SwitchId, Id) of
        not_found ->
            {error, #ofp_error_msg{type = group_mod_failed,
                                   code = unknown_group}};
        Group ->
            [group_reset_bucket_stats(SwitchId, B#linc_bucket.unique_id)
...
%%--------------------------------------------------------------------
-spec get_desc(integer(), #ofp_group_desc_stats_request{}) ->
                      #ofp_group_desc_stats_reply{}.
get_desc(SwitchId, #ofp_group_desc_stats_request{}) ->
    #ofp_group_desc_stats_reply{
       stats = group_enum_groups(SwitchId)
      }.
...
group_delete_2(SwitchId, Id, ProcessedGroups) ->
    case ordsets:is_element(Id, ProcessedGroups) of
        ...
        false ->
            ...
            linc_us3_flow:delete_where_group(SwitchId, Id),

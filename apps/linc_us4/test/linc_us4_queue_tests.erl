...
sending() ->
    PortId = 1,
    QueueId = 1,
    ?assertEqual(ok, linc_us4_queue:send(?SWITCH_ID, PortId, QueueId, <<>>)).
...
min_rate() ->
    ...
    ?assertEqual(ok,
                 linc_us4_queue:set_min_rate(?SWITCH_ID, PortId, QueueId, 500)).
...
max_rate() ->
    ...
    ?assertEqual(ok,
                 linc_us4_queue:set_max_rate(?SWITCH_ID, PortId, QueueId, 500)).

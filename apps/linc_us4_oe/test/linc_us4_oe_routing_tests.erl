...
%% @copyright 2012 FlowForwarding.org
-module(linc_us4_oe_routing_tests).
...
mask_match() ->
    [begin
         ...
         ?assertEqual(Result,
                      linc_us4_oe_routing:pkt_field_match_flow_field(Field1,
                                                                  Field2))
...
spawn_route() ->
    ?assert(is_pid(linc_us4_oe_routing:spawn_route(flow_table_and_pkt()))).

maybe_spawn_route() ->
    Pkt = flow_table_and_pkt(), 
    application:set_env(linc, sync_routing, false),
    ?assert(is_pid(linc_us4_oe_routing:maybe_spawn_route(Pkt))),
    application:set_env(linc, sync_routing, true),
    ?assertMatch({match, 0, _}, linc_us4_oe_routing:maybe_spawn_route(Pkt)).

match_with_and_without_vlan_tag() ->
    PktFields1 = create_ofp_fields([{vlan_vid, <<10:12>>}]),
    PktFields2 = [],
    FlowFields = [],
    ?assert(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields1,
                                                          FlowFields)),
    ?assert(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields2,
                                                          FlowFields)).
...
match_only_without_vlan_tag() ->
    ...
    ?assertNot(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields1,
                                                             FlowFields)),
    ?assert(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields2,
                                                          FlowFields)).
...
match_unrestricted_vlan_tag() ->
    ...
    ?assert(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields1,
                                                          FlowFields)),
    ?assertNot(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields2,
                                                             FlowFields)).
...
match_masked_vlan_tag() ->
    ...
    ?assert(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields2,
                                                          FlowFields)),
    ?assertNot(linc_us4_oe_routing:pkt_fields_match_flow_fields(PktFields3,
                                                             FlowFields)).
...
foreach_teardown(State) ->
    ok = linc_us4_oe_flow:terminate(State).
...
miss_no_flow_entries(TableConfig, MissError) ->
    ...
    TableId = 0,
    flow_table(TableId, FlowEntries, TableConfig),
    ?assertEqual({table_miss, MissError}, linc_us4_oe_routing:route(Pkt)).
